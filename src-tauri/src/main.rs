...
#[tauri::command]
async fn check_libreoffice(state: State<'_, AppState>) -> Result<bool, String> {
    let lo_manager = state.lo_manager.lock().await;
    Ok(lo_manager.is_available().await)
}

#[tauri::command]
async fn get_downloads_folder() -> Result<String, String> {
    dirs::download_dir()
...
async fn select_files(app: tauri::AppHandle) -> Result<Vec<String>, String> {
    let files = app.dialog()
        ...
        .set_title("Select Word Documents")
        .blocking_pick_files();
